...
# save file name
SAVE_DIR = FLAGS.train_dir + '_' +  FLAGS.model + '_' + FLAGS.atari_game + '_compress_' + '_seq_length_1'
...
def train():
  ...
  with tf.Graph().as_default():
    ...
    #y_2 = ring_net.lstm_compression(y_1, action[:,9,:], hidden_state, keep_prob_lstm)
    #x_2 = ring_net.decoding(y_2)

    x_2_o.append(x_2)
    # now collect values
    for i in xrange(9):
      #y_1 = ring_net.encoding(state[:,i+10,:,:,:], keep_prob_encoding)
      x_2, reward_2, hidden_state = ring_net.encode_compress_decode(state[:,i+10,:,:,:], action[:,i+11,:], hidden_state, keep_prob_encoding, keep_prob_lstm)
      x_2_o.append(x_2)
